...
//#include <sys/types.h> // check types.h chaunm
...
#ifdef __APPLE__ // those declaration here should not be use with lwip.chaunm change from ifndef APPLE to ifdef
...
#ifdef __APPLE__ // chaunm change ifndef -> ifdef
            else if (socket_io_instance->target_mac_address != NULL &&
