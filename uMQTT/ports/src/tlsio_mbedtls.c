...
static void mbedtls_init(void *instance, const char *host) {
    ...
    mbedtls_threading_set_alt( mutex_init_alt, mutex_free_alt,
        		mutex_lock_alt, mutex_unlock_alt ); // set alternate function for free RTOS - chaunm
    ...
    mbedtls_pk_init(&result->client_private_key_parsed);
    // end chaunm
    ...
    mbedtls_ssl_conf_authmode(&result->config, MBEDTLS_SSL_VERIFY_OPTIONAL); // chaunm - change from REQUIRE to OPTIONAL
    ...
    else
    {
        ...
        if (result != NULL)
        {
            ...
            else
            {
                ...
                result->client_certificates = NULL;
                result->client_private_key = NULL;
...
int tlsio_mbedtls_setoption(CONCRETE_IO_HANDLE tls_io, const char* optionName, const void* value)
{
    ...
    else
    {
        ...
        //chaunm add for client cert and password
        else if (strcmp(OPTION_X509_ECC_CERT, optionName) == 0)
        {
        	...
        	else
        	{
        		...
        		else
        		{
        			...
        			{
        				mbedtls_ssl_conf_own_cert(&tls_io_instance->config, &tls_io_instance->client_certificates_parsed, &tls_io_instance->client_private_key_parsed);
        			}
        			result = 0;
        		}
        	}
